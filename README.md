...
当前已经进入公司内部云端试用和维护阶段：登录访问、客户资料/PDF 解析、样例文章参考、多轮改稿、生成历史、引用情报分析和基础备份都已接入主流程。爬虫链路采用“云端任务中心 + 运营电脑本地 worker”试用方案，仍受本机登录态和平台风控影响，不承诺大规模无人值守稳定爬虫。
...
- `services/`：引用情报、crawl jobs、记录统计、内容提示词、深度分析等后端业务模块。
