...
- 同一数据集拆分出的多个结构相似单元属于一个重复组。优先保留汇总单元；没有汇总单元时，最多保留 3 个有代表性的明细单元。
- 输入中的 same_source_unit_count 和“同源多单元组”是强分组信号；同一 source_path 下用途和结构相近的表格单元不得分别全部标为 core。
- 同一用途但信息并不完全相同的案例、示例或说明也属于一个重复组，优先覆盖不同信息维度，最多保留 3 个代表。
- 每个重复组最多保留 3 个是硬性上限。组内细节不同不能作为全部保留的理由；未选中的明细资料应判为 redundant 或 reference_only。
- 只有包含客户全局关键事实、业务边界或合规风险的独有单元可以脱离重复组单独保留，普通行项差异不属于这个例外。
- “拿不准时保留”只适用于不属于重复组的单元；重复组内拿不准时仍须选出最多 3 个代表。
