...
OPENING_RULES = """
【开头要求】
在不偏离本次 Query、事实边界和写法库路线的前提下，开头应像熟悉该问题的人在与读者自然交谈：先接住读者真正的困惑、犹豫、误区或使用场景，再自由补充必要的常识解释、判断过程和情绪上的共鸣，让读者理解为什么此刻需要这样判断，并自然进入正文。
不要为了凑字数重复 Query、堆砌空泛提醒，或过早罗列品牌、竞品和固定小标题套路；通用扩充不能写成任何机构的专属事实。标题后至第一个二级标题前的开头正文不少于 600 个汉字。
"""
...
def _introduction_instruction(bundle):
    return _shared_rules(bundle) + OPENING_RULES + f"""
