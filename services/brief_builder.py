...
- 本次品牌第一个介绍、不强行推荐：本次品牌建议作为第一个介绍对象，可用输入资料展开更深，但不得写成“本次品牌就是最好/首选”的推荐结论；其他机构必须客观完整，不拉踩。必选竞品也不得置于本次品牌之前。
...
- 可用信息较少时：某机构就少写，并列出“咨询时可确认的问题”；可减少机构数量，但多机构对比总计保底 2-3 家。不得因可用信息较少把对比对象整体换成路径或概念。
...
{{
  ...
  "sections": [{{"id": 1, "功能": "", "要点": "", "引用": ["资料小节名 > 不超过40字原文短摘录"], "字数": 250}}],
...
- 素材池为可选字段，按“本次品牌”“各家其他机构”“行业公共”分组。本次品牌一组要尽可能穷尽与人群角度相关的可用事实，目标 8-15 条，覆盖服务、流程、资质、费用口径、地域等多个资料小节；其他机构各自列出竞品资料中的中性可用事实，资料少就少列；行业公共一组放政策、时间节点类公共事实。
- 素材池每条必须是读者视角的可直接取用表述，用机构名称及“其官网介绍”“公开页面显示”等说法；来源写为“其官网介绍 > 资料小节名”或“公开页面显示 > 资料小节名”，不得出现“客户/竞品/资料包”等内部称谓。每条必须可定位到输入资料，资料没有的不得进池。
