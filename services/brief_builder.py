...
def build_planning_brief_prompt(sample, *, customer_material_text="", content_upload_text="",
    ...
    prompt_sections = [
        {key: value for key, value in section.items() if key != "字数"}
        if isinstance(section, dict) else section
        for section in sections
    ]
    prompt_skeleton = copy.deepcopy(skeleton)
    prompt_skeleton.setdefault("payload", {})["sections"] = prompt_sections
...
【对比型硬结构】
- 多对比对象块必须存在：正文中间必须安排一大块“本次品牌 + 多家真实对比对象（机构或个人）”的逐个介绍与对比。其他对比对象只取自下方竞品资料中的真实名称；每个对象要给出定位、服务或专业侧重、适合人群和可核验信息，为用户提供选择依据。任何骨架、任何资料状况下都不得省略，也不得替换为路径、概念或学习形式对比。
- 本次品牌第一个介绍、不强行推荐：本次品牌建议作为第一个介绍对象，写成连贯的大段陈述而非条目罗列；可用输入资料展开更深，但不得写成“本次品牌就是最好/首选”的推荐结论；其他机构或个人必须客观完整，不拉踩。必选竞品也不得置于本次品牌之前。
- 不得使用推荐等级词汇和分档标签：禁止最推荐、首推、强烈推荐、推荐指数、第一/第二/第三梯队、按需选择、谨慎考察、星级及任何对比对象的推荐档次。机构或个人只能按服务模式、专业方向、业务侧重或地域覆盖等中性维度分组；组的呈现顺序不代表排名，正文必须明示这一点。
- 本次品牌可第一个介绍且篇幅更深，但与其他机构或个人使用同样的字段框架，不得借档位、定性词或章节标题制造高下。骨架自带排名、梯队或打分语义时，保留维度权重、横向对比表和来源声明等量化外壳，分档改写为中性分组。
- 其他机构或个人在资料允许时充分展开；可用信息较少时才少写并列出“咨询时可确认的问题”；可减少对比对象数量，但对比对象总计保底 2-3 家。不得因可用信息较少把对比对象整体换成路径或概念。
- combo_warning 只用于报告缝合困难，不是改变文章对象类型、替换多对比对象块或省略该对比块的授权。
""" if (skeleton.get("payload") or {}).get("parent_type") == "对比型" else ""
    return f"""你是 GEO 内容的策划简报 LLM。只输出 JSON，不要输出 Markdown、解释或成品文章。

你的职责是把已经抽签确定的文章形状，结合输入资料，写成施工简报。不得输出可直接发布的成品句子；“要点”只能写施工指令：写什么、采用什么资料、什么口径以及资料缺口提示。给写作层的指导涉及对比对象时一律使用真实对象名称；对比对象可以是机构或个人。客户/竞品是内部称谓，不得出现在成文。信息缺口只能指示省略维度或改写为读者核验动作，不得指示写“未提供”“资料缺失”“待补充”。

【输出 JSON schema】
“字数”仅为兼容旧简报保留的字段，填写任意整数，不用于限制成文篇幅。
{{
  "title_candidates": ["2-3 个标题候选"],
  "angle_statement": "人群角度贯穿全文的一句主线",
  "sections": [{{"id": 1, "功能": "", "要点": "", "引用": ["资料小节名 > 原文短摘录"], "字数": 0, "展开来源": ["客户资料包 > 产品与服务"]}}],
  "素材池": {{"对比对象名称或行业公共": [{{"表述": "读者视角的一句可用事实", "来源": "资料小节名"}}]}},
  "bans": ["禁用项"],
...
- 本次品牌的介绍或展开节必须给出“展开来源”，指向客户资料包中真实存在的小节名（可多个），并在“要点”写清展开角度；多对比对象节中每家其他机构或个人也必须给出“展开来源”，指向竞品 Markdown 的对应对象分节。展开来源只能指向输入资料中真实存在的小节名/对比对象名称，不得发明。
- 本次品牌的介绍或展开节必须在“要点”提炼 2-4 个本篇主打的 GEO 宣传点。每个宣传点只写“主题一句话 + 对应展开来源 + 展开角度”，是施工指令，不是成品句；禁止写成可直接抄进正文的成品句。宣传点从核心优势、产品与服务、信任凭证等真实小节提炼，并与人群角度呼应；品牌节的展开来源必须与宣传点对应。
- 对比型的需求背景、政策背景、编者按或背景类章节只写时间或政策锚点式短段，不得扩写成大段行业科普；“行业公共背景”类小节不得作为品牌节的展开来源，只能在“引用”字段短摘录作为背景锚点。
- 素材池为可选字段，是保底清单而非唯一取材通道；按“本次品牌”“各家其他机构或个人”“行业公共”分组。本次品牌一组要尽可能穷尽与人群角度相关的可用事实，目标 8-15 条，覆盖服务、流程、资质、费用口径、地域等多个资料小节；其他机构或个人各自列出竞品资料中的中性可用事实，资料少就少列；行业公共一组放政策、时间节点类公共事实。池内条目鼓励用足，写作层也可按“展开来源”直接取材。
...
【抽样骨架】
{json.dumps(prompt_skeleton, ensure_ascii=False)}

【骨架 section 顺序】
{json.dumps(prompt_sections, ensure_ascii=False)}
