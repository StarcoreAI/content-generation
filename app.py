...
def precise_diagnosis():
    ...
    return jsonify({"questions": build_precise_diagnosis(records)})
...
def precise_question_refs():
    ...
    records = load_client_records(client_id, group_id=group_id,
                                  platform=platform if platform else None)
    records = filter_records_by_question(records, question_filter)
    if not records:
        return jsonify({"total": 0, "top_articles": [], "platform_dist": [], "ai_insight": ""})

    ref_stats = build_precise_question_ref_stats(records)
    top_articles = ref_stats["top_articles"]
    platform_dist = ref_stats["platform_dist"]
