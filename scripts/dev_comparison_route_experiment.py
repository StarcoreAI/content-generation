...
from services.comparison_route_experiment import run_comparison_route_experiment, validate_comparison_route_bundle
...
def run_manual_comparison_route_experiment(bundle, customer_master_text, output_dir, writer_ai_fn):
    ...
    validated = validate_comparison_route_bundle(experiment_bundle)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "draft.md").write_text(result["draft"], encoding="utf-8")
    trace = {
        "schema_version": 1,
        "query": validated["task"]["query"],
        "route_name": validated["selected_route"]["name"],
        "customer_master_characters": len(validated["customer_master_text"]),
        "competitor_names": [item["name"] for item in validated["competitors"]],
        "title_entity_policy": validated["task"]["title_entity_policy"],
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Run a manual comparison content-route experiment.")
    parser.add_argument("--input", required=True, help="Article task, route, and explicit competitor facts JSON path")
    ...
    result = run_manual_comparison_route_experiment(bundle, customer_master_text, args.output_dir, geo_app.ai)
